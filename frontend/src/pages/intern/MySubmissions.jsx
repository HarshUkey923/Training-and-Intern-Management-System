...
const themes = {
  dark: {
    ...
    feedbackBg: "rgba(16,185,129,0.06)",
  ...
  light: {
    ...
    feedbackBg: "rgba(16,185,129,0.05)",
    feedbackBorder: "rgba(16,185,129,0.2)",
  },
};
...
const MySubmissions = () => {
  ...
  const avgRating = submissions.filter(s => s.rating).length
    ? (submissions.reduce((a, s) => a + (s.rating || 0), 0) / submissions.filter(s => s.rating).length).toFixed(1)
    : "—";
      ...
      {!loading && submissions.length > 0 && (
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginBottom: "20px" }}>
          {[
            ...
            <div key={p.label} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "6px 14px", borderRadius: "20px", background: p.bg, border: `1px solid ${p.color}30` }}>
              <span style={{ fontSize: "13px", fontWeight: 700, color: p.color }}>{p.value}</span>
              <span style={{ fontSize: "11px", color: t.textMuted }}>{p.label}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <div
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px", marginBottom: sub.note || sub.feedback ? "14px" : "0" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
          ...
          {/* File link */}
          {sub.fileUrl && (
            <a href={`http://localhost:5001/${sub.fileUrl}`} target="_blank" rel="noreferrer"
              style={{ display: "flex", alignItems: "center", gap: "4px", fontSize: "11px", color: "#818cf8", textDecoration: "none" }}
              title="View attachment">
              <ExternalLinkIcon size={12} /> File
            </a>
          )}
        </div>
      </div>
