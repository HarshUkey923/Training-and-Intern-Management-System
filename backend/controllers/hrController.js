...
// ─── Add HR — only callable by authenticated HR ───────────────────────────────
export const AddHR = async (req, res) => {
    ...
    try {
        ...
        const hashed = await bcrypt.hash(password, 10);
        const user   = await User.create({ name, email, password: hashed, role: "HR" });
        res.status(201).json({ message: "HR account created.", user: { _id: user._id, name: user.name, email: user.email } });
    } catch (error) { res.status(500).json({ message: error.message }); }
};
